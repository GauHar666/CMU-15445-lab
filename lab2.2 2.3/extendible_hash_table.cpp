...
//利用哈希函数获得key哈希之后的返回值，转化为uint32_t类型返回。也就是索引的最高位，最后两位这样的，这个几位和全局深度的大小是一样的。
template <typename KeyType, typename ValueType, typename KeyComparator>
auto HASH_TABLE_TYPE::Hash(KeyType key) -> uint32_t {
  return static_cast<uint32_t>(hash_fn_.GetHash(key));
}

//hash完和他的全局深度按位与一下，就可以得到桶的id
template <typename KeyType, typename ValueType, typename KeyComparator>
...
inline auto HASH_TABLE_TYPE::KeyToPageId(KeyType key, HashTableDirectoryPage *dir_page_) -> uint32_t {
  uint32_t index = KeyToDirectoryIndex(key, dir_page_); //index是桶id
  page_id_t page_id = dir_page_->GetBucketPageId(index); //底层是调用了pages数组里面用桶id或者pageid的方法。
  return page_id;
}
//创建目录页和桶页面都是用了buffer_pool_manager内部的NewPage方法。
template <typename KeyType, typename ValueType, typename KeyComparator>
...
auto HASH_TABLE_TYPE::CreateBucketPage(page_id_t *bucket_page_id) -> HASH_TABLE_BUCKET_TYPE * {
  auto *new_bucket_page =
      reinterpret_cast<HASH_TABLE_BUCKET_TYPE *>(buffer_pool_manager_->NewPage(bucket_page_id, nullptr)->GetData()); //创建新的桶的页面，桶页面可能很多，目录页只有一个。
  return new_bucket_page;
}

//同理fetchdirectorypage和fetchbucketpage都是使用了buffer_pool_manager内部的fetchpage函数
template <typename KeyType, typename ValueType, typename KeyComparator>
auto HASH_TABLE_TYPE::FetchDirectoryPage() -> HashTableDirectoryPage * {
  ...
  auto directory_page = reinterpret_cast<HashTableDirectoryPage *>(buffer_pool_manager_->FetchPage(directory_page_id_,nullptr)->GetData());
  return directory_page;
}
...
auto HASH_TABLE_TYPE::GetValue(Transaction *transaction, const KeyType &key, std::vector<ValueType> *result) -> bool {
  table_latch_.RLock();//RLock表示的是读锁，对下面的代码上读锁。
  ...
  reinterpret_cast<Page *>(bucket_page)->RUnlatch();//释放锁，唤醒线程。
...
auto HASH_TABLE_TYPE::Insert(Transaction *transaction, const KeyType &key, const ValueType &value) -> bool {
  table_latch_.RLock();
  page_id_t bucket_page_id = KeyToPageId(key, dir_page_);//通过桶id获得pageid
  auto *bucket_page = FetchBucketPage(bucket_page_id); //拿出一页page

  reinterpret_cast<Page *>(bucket_page)->WLatch(); //写锁
  bool ret = bucket_page->Insert(key, value, comparator_); //page的方法insert进去。
...
auto HASH_TABLE_TYPE::SplitInsert(Transaction *transaction, const KeyType &key, const ValueType &value) -> bool {
  ...
  uint32_t old_bucket_page_index = KeyToDirectoryIndex(key, dir_page_);//获取桶的id
  ...
  if (!old_bucket_page->IsFull()) {  // 再次检查桶是否满了,其实这一步可以省略，如果没满估计都不会进入这步
  ...
  uint32_t new_local_mask = old_local_mask + (old_local_mask + 1);  // 计算新的掩码，比之前多一位，例如1111。相当于翻倍+1
  uint32_t old_local_hash = old_bucket_page_index & new_local_mask;  // 分裂后旧桶对应的hash值 例1011
  uint32_t new_local_hash = old_local_hash ^ (old_local_mask + 1);  // 分裂后新桶对应的hash值 例0011，对其最高位取反，异或获取新
  uint32_t dir_size = dir_page_->Size();

  // 首先遍历一遍目录，将仍指向旧桶的位置深度加一
  // for (uint32_t i = 0; i < dir_size; i++) {
  //   if ((i & new_local_mask) == old_local_hash) { //如果还指向原来的旧桶的位置，那么这里深度+1
  ...
  if (local_depth < dir_page_->GetGlobalDepth()) {  // 不影响目录大小，只是将一半指向旧桶的指针改向新桶
    ...
    dir_page_->IncrGlobalDepth();//全局深度+1
    uint32_t new_dir_size = dir_page_->Size();//目录页数目翻倍
...
void HASH_TABLE_TYPE::Merge(Transaction *transaction, const KeyType &key, const ValueType &value) {
  ...
  page_id_t bucket_page_id = KeyToPageId(key, dir_page_);
  uint32_t dir_size = dir_page_->Size();
  uint32_t local_depth = dir_page_->GetLocalDepth(index);         // 该索引在目录中的局部深度，例如为3
  uint32_t old_local_mask = dir_page_->GetLocalDepthMask(index);  // 该索引的合并前掩码，例如为111
  uint32_t new_local_mask = old_local_mask ^ (1 << (local_depth - 1));  // 合并后的掩码，将最高位的1去掉，例如为11，因为合并所以位数要减小
  ...
  if (local_depth > 0 && bucket_page->IsEmpty()) {  // remove函数加的是读锁，有可能已经插入新值
    ...
    if (merge_occur) {
      // for (uint32_t i = 0; i < dir_size; i++) {
      //   if ((i & old_local_mask) == (index & old_local_mask)) {  
      ...
      for (uint32_t i = old_local_hash; i < dir_size; i += old_local_mask + 1) {
        dir_page_->SetBucketPageId(i, another_bucket_page_id);// 寻找指向空桶的指针,将其指向另一半another_bucket
      }
      ...
      // 与上面代码实现一样的功能
      for (uint32_t i = new_local_hash; i < dir_size; i += new_local_mask + 1) {
        dir_page_->DecrLocalDepth(i); //桶的深度减小
      }
...
// 循环合并
/*
在进行额外合并时，会将两个深度相等且一个为空的桶合并成一个桶，但是有可能在这个过程中，这两个桶的某些位置上可能存在相同的key值，
因此需要将这些key值进行重新散列，并放入新桶中
*/
template <typename KeyType, typename ValueType, typename KeyComparator>
